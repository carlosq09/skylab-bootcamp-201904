...
app.get('/home/search', checkLogin('/', false), urlencodedParser, (req, res) => {
    ...
    .then( favs => {
        logic.searchDucks(query)
            .then(ducks => {
                ducks = ducks.map(({ id, title, imageUrl: image, price }) => ({ url: `/home/duck/${id}`, id, title, image, price }))
...
app.get('/home/favs', checkLogin('/', false), urlencodedParser, (req, res) => {
    let { query: { query }, logic, session } = req

    
    query? session.query = query : query=session.query
...
app.post('/home/fav/:id', [checkLogin('/', false), urlencodedParser], (req, res) => {
